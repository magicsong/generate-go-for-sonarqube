...
func prepare(pkgName, workingDir, endpoint, username, password string, notest bool) error {
	...
	NoTest = notest
	if NoTest {
		glog.V(0).Infoln("Will not generate test files")
	}
...
func Build(pkgName, workingDir, endpoint, username, password string, noTest bool, apidoc *api.API) error {
	err := prepare(pkgName, workingDir, endpoint, username, password, noTest)
	if err != nil {
		return err
	}
	// add static file
...
func GenerateServiceActionContent(serviceName string, action *api.Action) *Statement {
	...
	if n, ok := converter.GetNameConvert()[respName]; ok {
		glog.V(3).Infof("Auto change %s to %s as they are same", respName, n)
		respName = n
	}
	if len(action.Params) == 0 {
		hasOption = false
